"""Train 14B on ALL data (7899) for submission. Quick: ~2 hours on 1 GPU."""
...
OUTPUT_DIR = PROJECT_ROOT / "models" / "qwen25-14b-lora-alldata"
...
def main():
    ...
    with open(TRAIN_FILE) as f: data = json.load(f)
    with open(TRIAL_FILE) as f: data += json.load(f)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME, device_map={"": 0}, trust_remote_code=True, torch_dtype=torch.bfloat16)
    ...
    print("Training 14B on ALL data")
    start = time.time()
    trainer.train()
