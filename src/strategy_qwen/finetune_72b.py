"""Fine-tune Qwen2.5-72B-Instruct-AWQ with LoRA on 2 GPUs.
AWQ is already 4-bit quantized, so we just add LoRA on top.
"""
...
OUTPUT_DIR = PROJECT_ROOT / "models" / "qwen25-72b-lora"
...
def main():
    ...
    lora_config = LoraConfig(
        r=16, lora_alpha=16,  # conservative rank for 72B
    ...
    training_args = SFTConfig(
        ...
        fp16=True,  # AWQ model is in fp16
    ...
    print("Training: rank=16, epochs=3, lr=1e-4, batch=1x16")
    start = time.time()
    trainer.train()
