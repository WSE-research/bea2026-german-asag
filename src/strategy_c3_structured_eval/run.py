"""
Strategy C3: Structured Rubric Evaluation + Few-Shot

Forces model to evaluate each rubric criterion before scoring.

Usage:
    python -m src.strategy_c3_structured_eval.run [--split train|trial] [--workers 5] [--limit N] [--offset N]
"""
...
from src.strategy_c3_structured_eval.scorer import configure, score_sample

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
RESULTS_DIR = PROJECT_ROOT / "results" / "strategy_c3"
...
def score_one(sample: dict) -> dict:
    ...
    try:
        ...
        result["criteria_met"] = pred.get("criteria_met", [])
        result["criteria_missed"] = pred.get("criteria_missed", [])
...
def main():
    parser = argparse.ArgumentParser(description="Strategy C3: Structured Evaluation")
    ...
    print("=" * 70)
    print("  Strategy C3: Structured Rubric Evaluation + Few-Shot")
    print("=" * 70)
    print(f"  Split: {args.split} | Workers: {args.workers} | Offset: {args.offset} | Limit: {args.limit or 'all'}")
    print(f"  Examples/label: {args.examples_per_label} | Seed: {args.seed} | Model: {get_model()}")
    print("=" * 70)
    ...
    logging.getLogger(__name__).info("Scoring %d samples", len(data))
    ...
    meta = {"strategy": "C3: Structured Eval", "split": args.split, "model": get_model(),
            "offset": args.offset, "n_samples": len(data), "elapsed_seconds": round(elapsed_total, 2),
            "metrics": metrics}
