"""
Single-sample scorer for Strategy A (Rubric-Only).

Takes an ALICE dataset sample and returns a scored result dict
by calling the LLM via OpenRouter with the rubric-only prompt.
"""

import logging

from src.common.openrouter import call_with_retry
from src.strategy_a_rubric_only.prompt import (
    build_system_prompt,
    build_user_prompt,
    parse_response,
)
...
def score_sample(sample: dict) -> dict:
    """
    Score a single ALICE sample using the rubric-only strategy.
