"""
Strategy C5d: Rubric Decomposition

Decomposes rubric evaluation into binary sub-criteria checks, then aggregates
into the final 3-way score. Instead of asking the LLM for a holistic label,
this strategy instructs the model to identify individual criteria from the
"Correct" rubric and evaluate each as fulfilled/not fulfilled.

Usage:
    python -m src.strategy_c5d_decomposed.run [--split train|trial|test] [--workers 5] [--limit N] [--n-boundary 1] [--n-similar 1] [--seed 42] [--offset N]
"""
...
from src.strategy_c5d_decomposed.scorer import configure, score_sample

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
RESULTS_DIR = PROJECT_ROOT / "results" / "strategy_c5d"
...
def score_one(sample: dict) -> dict:
    ...
    try:
        ...
        result["criteria_fulfilled"] = pred.get("criteria_fulfilled")
        result["criteria_total"] = pred.get("criteria_total")
...
def compute_metrics(results: list[dict]) -> dict:
    ...
    # C5d-specific: criteria decomposition stats
    criteria_stats = _compute_criteria_stats(valid)

    metrics = {"n_scored": len(valid), "n_errors": len(results) - len(valid),
               "accuracy": round(accuracy, 4), "macro_f1": round(macro_f1, 4),
               "weighted_f1": round(weighted_f1, 4), "qwk": round(qwk, 4), "per_class": per_class}
    if criteria_stats:
        metrics["criteria_stats"] = criteria_stats
    return metrics


def _compute_criteria_stats(valid: list[dict]) -> dict | None:
    """Compute statistics about criteria decomposition quality."""
    with_criteria = [r for r in valid if r.get("criteria_fulfilled") is not None and r.get("criteria_total") is not None]
    if not with_criteria:
        return None

    totals = [r["criteria_total"] for r in with_criteria]
    fulfilled = [r["criteria_fulfilled"] for r in with_criteria]
    ratios = [f / t if t > 0 else 0.0 for f, t in zip(fulfilled, totals)]

    return {
        "n_with_criteria": len(with_criteria),
        "avg_criteria_total": round(sum(totals) / len(totals), 2),
        "avg_criteria_fulfilled": round(sum(fulfilled) / len(fulfilled), 2),
        "avg_fulfillment_ratio": round(sum(ratios) / len(ratios), 4),
    }


def main():
    parser = argparse.ArgumentParser(description="Strategy C5d: Rubric Decomposition")
    ...
    parser.add_argument("--n-boundary", type=int, default=1, help="Boundary examples per adjacent boundary")
    ...
    print("=" * 70)
    print("  Strategy C5d: Rubric Decomposition")
    print("=" * 70)
    ...
    if "error" not in metrics:
        ...
        if "criteria_stats" in metrics:
            cs = metrics["criteria_stats"]
            print(f"\n  Criteria decomposition: {cs['n_with_criteria']}/{metrics['n_scored']} samples")
            print(f"    Avg criteria/question: {cs['avg_criteria_total']:.1f} | Avg fulfilled: {cs['avg_criteria_fulfilled']:.1f}")
            print(f"    Avg fulfillment ratio: {cs['avg_fulfillment_ratio']:.2%}")
    else:
    ...
    meta = {"strategy": "C5d: Rubric Decomposition", "split": args.split, "model": get_model(),
