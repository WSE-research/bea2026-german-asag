"""
Strategy C5a: Post-Processing + Anti-Examples

Extends C4 (smart example selection) with:
- Anti-examples in the system prompt to reduce common scoring mistakes
- Post-processing rules to override scores for short/empty/refusal answers

Usage:
    python -m src.strategy_c5a_postprocess.run [--split train|trial|test] [--workers 5] [--limit N] [--n-boundary 2] [--n-similar 1] [--seed 42] [--offset N]
"""
...
from src.strategy_c5a_postprocess.scorer import configure, score_sample

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
RESULTS_DIR = PROJECT_ROOT / "results" / "strategy_c5a"
...
def score_one(sample: dict) -> dict:
    ...
    try:
        ...
        result["overridden"] = pred.get("overridden", False)
...
def main():
    parser = argparse.ArgumentParser(description="Strategy C5a: Post-Processing + Anti-Examples")
    ...
    print("=" * 70)
    print("  Strategy C5a: Post-Processing + Anti-Examples")
    print("=" * 70)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            if completed % 25 == 0 or completed == len(data):
                ...
                overrides = sum(1 for r in results if r.get("overridden"))
                print(f"  [{completed:>5d}/{len(data)}] {rate:.1f} samples/s | errors: {errors} | overrides: {overrides} | elapsed: {elapsed:.1f}s")
    ...
    n_overridden = sum(1 for r in results if r.get("overridden"))
    print("\n" + "=" * 70)
    print("  Results — C5a: Post-Processing + Anti-Examples")
    print("=" * 70)
    if "error" not in metrics:
        print(f"  QWK: {metrics['qwk']:.4f} | Acc: {metrics['accuracy']:.4f} | wF1: {metrics['weighted_f1']:.4f}")
        print(f"  Scored: {metrics['n_scored']} | Errors: {metrics['n_errors']} | Post-process overrides: {n_overridden}")
    ...
    meta = {"strategy": "C5a: Post-Processing + Anti-Examples", "split": args.split, "model": get_model(),
            ...
            "n_overridden": n_overridden, "metrics": metrics}
